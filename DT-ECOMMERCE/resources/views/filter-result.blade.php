                  ...
                  <div class="list-group" id="categoryList">
                      @foreach ($categories as $index => $category)
                          <label class="list-group-item{{ $index >= 12 ? ' d-none' : '' }}">
                              <input class="form-check-input me-1" type="checkbox" name="categories[]" value="{{ $category }}" {{ in_array($category, $request->categories ?? []) ? 'checked' : '' }}>
                              {{ $category }}
                          </label>
                      @endforeach
                  </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <input value="{{ $request->min_price }}" type="number" class="form-control" id="min" name="min_price" placeholder="Min">
                        </div>
                        <div class="col">
                            <label for="max" class="form-label">Max</label>
                            <input value="{{ $request->max_price }}" type="number" class="form-control" id="max" name="max_price" placeholder="Max">
                        </div>
                    </div>
                </div>
                <div class="mb-3">
            ...
            <div class="row justify-content-center mt-5">
                <div class="col-md-6 text-center">
                    @if($processedCollection->isEmpty())
                        <p>No Item(s) or Package(s) Found</p>
                    @endif
                </div>
            </div>
            <div class="row">
          ...
          <nav aria-label="Page navigation example">
              <ul class="pagination justify-content-center">
                  ...
                  <li class="page-item <?= ($currentPage == 1) ? 'disabled' : '' ?>">
                      <a class="page-link" href="{{ route('search', array_merge(request()->query(), ['page' => 1])) }}">First</a>
                  </li>
  
                  <!-- Link to the previous page -->
                  <li class="page-item <?= ($currentPage <= 1) ? 'disabled' : '' ?>">
                      <a class="page-link" href="{{ route('search', array_merge(request()->query(), ['page' => max($currentPage - 1, 1)])) }}">Previous</a>
                  </li>
  
                  <!-- Dynamic page links -->
                  @for ($i = max(1, $currentPage - 10); $i <= min($totalPages, $currentPage + 10); $i++)
                      <li class="page-item <?= ($i == $currentPage) ? 'active bg-danger' : '' ?>">
                          <a class="page-link" href="{{ route('search', array_merge(request()->query(), ['page' => $i])) }}">{{ $i }}</a>
                      </li>
                  @endfor
  
                  <!-- Link to the next page -->
                  <li class="page-item <?= ($currentPage >= $totalPages) ? 'disabled' : '' ?>">
                      <a class="page-link" href="{{ route('search', array_merge(request()->query(), ['page' => min($currentPage + 1, $totalPages)])) }}">Next</a>
                  </li>
  
                  <!-- Link to the last page -->
                  <li class="page-item <?= ($currentPage == $totalPages) ? 'disabled' : '' ?>">
                      <a class="page-link" href="{{ route('search', array_merge(request()->query(), ['page' => $totalPages])) }}">Last</a>
                  </li>
              </ul>
          </nav>
      </div>
