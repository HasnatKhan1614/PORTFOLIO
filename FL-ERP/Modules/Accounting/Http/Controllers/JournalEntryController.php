...
class JournalEntryController extends Controller
{
    ...
    public function index()
    {
        ...
        $costCenters = AccountingAccountCostCenter::where('business_id',$business_id)->pluck('name', 'id')->toArray();
        ...
        if (request()->ajax()) {
            $journal = AccountingAccTransMapping::where('accounting_acc_trans_mappings.business_id', $business_id)
                ...
                ->join('accounting_account_cost_centers as c', 'accounting_acc_trans_mappings.accounting_account_cost_center_id', '=', 'c.id')
            ...
            if (!empty(request()->accounting_account_cost_center_id)) {
                $accounting_account_cost_center_id = request()->accounting_account_cost_center_id;
                $journal->where('accounting_acc_trans_mappings.accounting_account_cost_center_id', '=', $accounting_account_cost_center_id);
            }
                ...
                ->addColumn(
                    'action', function ($row) {
                        ...
                        if (auth()->user()->can('accounting.view_journal')) {
                            $html .= '<li>
                                    <a href="' . action([\Modules\Accounting\Http\Controllers\JournalEntryController::class, 'show'], [$row->id]) . '" data-href="">
                                        <i class="fas fa-eye" aria-hidden="true"></i>'.__("View").'
                                    </a>
                                    </li>';
                        }
        ...
        return view('accounting::journal_entry.index',compact('costCenters'));
    }
    ...
    public function create()
    {
        ...
        return view('accounting::journal_entry.create')->with(compact( 'costCenters'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // $accounting_account_cost_center_ids = $request->get('accounting_account_cost_center_id');
            $accounting_account_cost_center_id = $request->get('accounting_account_cost_center_id');
            ...
            $acc_trans_mapping->accounting_account_cost_center_id = $accounting_account_cost_center_id;
            ...
            foreach ($account_ids as $index => $account_id) {
                if (! empty($account_id)) {
                    ...
                    // if (!empty($accounting_account_cost_center_ids[$index])) {
                    //     $transaction_row['accounting_account_cost_center_id'] = $accounting_account_cost_center_ids[$index];
                    // }
    ...
    public function show($id)
    {
        ...
        if (request()->ajax()) {
            $query = DB::table('accounting_accounts_transactions')
                ->where('accounting_acc_trans_mappings.id', $id)
                ->where('accounting_acc_trans_mappings.business_id', $business_id)
                ->join('accounting_acc_trans_mappings', 'accounting_accounts_transactions.acc_trans_mapping_id', '=', 'accounting_acc_trans_mappings.id')
                ->leftJoin('accounting_account_cost_centers as c', 'accounting_acc_trans_mappings.accounting_account_cost_center_id', '=', 'c.id')
                ...
                ->select([
                    ...
                    'accounting_accounts_transactions.type as type', // 👈 add this line explicitly
            ...
            if (!empty(request()->start_date) && !empty(request()->end_date)) {
                $start = request()->start_date;
                $end = request()->end_date;
                $query->whereDate('accounting_accounts_transactions.operation_date', '>=', $start)
                    ->whereDate('accounting_accounts_transactions.operation_date', '<=', $end);
            }
            
            if (!empty(request()->accounting_account_cost_center_id)) {
                $accounting_account_cost_center_id = request()->accounting_account_cost_center_id;
                $query->where('accounting_acc_trans_mappings.accounting_account_cost_center_id', '=', $accounting_account_cost_center_id);
            }

            return Datatables::of($query) // ✅ pass query builder directly
                ->addIndexColumn()
                ->make(true);
        }

        return view('accounting::journal_entry.view_items', compact('costCenters', 'itemId'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('accounting::journal_entry.edit')
            ->with(compact('journal', 'accounts_transactions','costCenters'));
    }
